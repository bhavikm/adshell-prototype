...
<h4 class="red-text"><a class="red-text"  href="index.php?employee&action=managefuel">Manage Customer Fuel Cards</a> >  <?php echo $data['businessName']; ?></h4>
<br />
	...
	<tbody>
		...
		<tr>
			...
			<td>
				...
				<br /><br />
				<a class="btn btn-danger bottomOfTableCell" href="index.php?employee&action=managefuel&second=resetpin&appid=<?php echo $fuelCard['applicationID']; ?>&cardid=<?php echo $fuelCard['fuelCardID']; ?>">Reset &raquo;</a>
				<?php } ?>
				</div>
			</td>
			<?php } else { ?>
			...
			<td>
				...
				<?php } ?>
				<?php if ($fuelCard['cardStatus'] != 'cancelled') { ?>
				<br /><a class="btn btn-primary bottomOfTableCell" href="index.php?employee&action=managefuel&second=changeproducts&appid=<?php echo $fuelCard['applicationID']; ?>&cardid=<?php echo $fuelCard['fuelCardID']; ?>">Edit &raquo;</a>
				<?php } ?>
				</div>
			</td>
			...
			<?php } ?>
				<div class="innerTableCellWrapper">
					...
					<br /><br /><a class="btn btn-primary bottomOfTableCell" href="index.php?employee&action=managefuel&second=changeproducts&appid=<?php echo $fuelCard['applicationID']; ?>&cardid=<?php echo $fuelCard['fuelCardID']; ?>">Update &raquo;</a>
				</div>
			</td>
		</tr>
		<?php } ?>
...
$(document).ready(function(){
	  ...
	  $(document).on('click','#addCardHolder',function(event) {
		...
		var part1 = '<form class="form" method="post" role="form" id="cardForm" action="index.php?employee"><input type="hidden" class="form-control" name="action" id="action" value="managefuel"><input type="hidden" class="form-control" name="second" id="second" value="addcard"><input type="hidden" class="form-control" name="appid" id="appid" value="<?php echo $fuelCard['applicationID']; ?>">';
		
		var part2 = '<div class="row form-subheading"><div class="col-md-12"><h4>Add Cardholder</h4></div></div>';
